...
const formattedInputs = inputs.split('\n\n').map(str => str.split(/\s/g))

const main = (inputs) => {
  let total = 0;
  for (const strings of inputs) {
    let set = new Set(strings[0]);
    for (let i = 1; i < strings.length; i++) {
      const string = strings[i];
      set = new Set(
        [...string.split('')].filter(char => set.has(char))
      )
    }
    total += set.size
  }
